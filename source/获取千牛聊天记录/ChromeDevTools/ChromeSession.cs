...
namespace MasterDevs.ChromeDevTools
{
    public class ChromeSession : IChromeSession
    {
        ...
        public ChromeSession(string endpoint, ICommandFactory commandFactory, ICommandResponseFactory responseFactory, IEventFactory eventFactory, string title)
        {
            _endpoint = endpoint;
        ...
        public ICommandResponse SendCommand<T>()
        {
            Command command = this._commandFactory.Create<T>();
            return this.SendCommandInner(command); ;
        }
        public ICommandResponse SendCommand<T>(T parameter)
        {
            if (parameter == null) return SendCommand<T>();
            Command command = this._commandFactory.Create<T>(parameter);
            return this.SendCommandInner(command); ;
        }

        private ICommandResponse SendCommandInner(Command command)
        {
            ...
            if (!requestResetEvent.Wait(3000))
            {
                throw new Exception("SendCommand超时，command=" + command.ToString());
            }
        ...
        private ICommandResponse SendCommand(Command command)
        {
            var settings = new JsonSerializerSettings
            {
                ContractResolver = new MessageContractResolver(),
        ...
        public Task<CommandResponse<T>> SendAsync<T>(ICommand<T> parameter, CancellationToken cancellationToken)
        {
            ...
            var task = SendCommandAsync(command, cancellationToken);
        ...
        private Task<ICommandResponse> SendCommandAsync(Command command, CancellationToken cancellationToken)
        {
            var settings = new JsonSerializerSettings
            {
                ContractResolver = new MessageContractResolver(),
